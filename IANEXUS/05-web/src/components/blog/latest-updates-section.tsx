...
export default function LatestUpdatesSection({
  ...
  return (
    <section className="overflow-hidden rounded-[1.5rem] border border-white/80 bg-white/92 shadow-[0_24px_70px_rgba(15,23,42,0.06)]">
      <div className="grid lg:grid-cols-[0.95fr_1.05fr]">
        <div className="border-b border-slate-200/80 p-5 md:p-6 lg:border-b-0 lg:border-r">
          ...
          <Link
            href="/blog"
            className="mt-7 inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-slate-300 hover:text-slate-950"
          >
        ...
        <div className="grid gap-px bg-slate-200 md:grid-cols-3">
          {posts.map((post, index) => (
            <Link
              key={post.id}
              href={`/blog/${post.slug}`}
              className="group bg-white p-4 transition duration-200 hover:bg-slate-50"
            >
              ...
              <h3 className="mt-4 line-clamp-2 text-sm font-semibold leading-snug text-slate-950">
                {post.title}
              </h3>
