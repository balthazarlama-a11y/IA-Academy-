...
export default function LatestUpdatesSection({
  ...
  return (
    <section className="overflow-hidden rounded-[2rem] border border-white/80 bg-white/92 shadow-[0_24px_70px_rgba(15,23,42,0.06)]">
      <div className="grid lg:grid-cols-[0.95fr_1.05fr]">
        <div className="border-b border-slate-200/80 p-6 md:p-8 lg:border-b-0 lg:border-r">
          ...
          <Link
            href="/blog"
            className="mt-7 inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:border-slate-300 hover:text-slate-950"
          >
        ...
        <div className="grid gap-px bg-slate-200 md:grid-cols-3">
          {posts.map((post, index) => (
            <Link
              key={post.id}
              href={`/blog/${post.slug}`}
              className="group bg-white p-5 transition duration-200 hover:bg-slate-50"
            >
              ...
              <h3 className="mt-4 line-clamp-3 text-base font-semibold leading-snug text-slate-950">
                {post.title}
              </h3>
              ...
              <div className="mt-5 flex items-center gap-2 border-t border-slate-200 pt-3 text-[11px] text-slate-500">
