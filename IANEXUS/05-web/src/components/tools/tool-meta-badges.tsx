...
export default function ToolMetaBadges({ tool }: { tool: Tool }) {
  ...
  const statusLabel = tool.edu_verified
    ? {
        icon: GraduationCap,
        className:
          "border-cyan-300/30 bg-cyan-400/10 text-cyan-700",
        text: "Verificación académica",
      }
    : tool.verified
      ? {
          icon: BadgeCheck,
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      <span className="inline-flex items-center gap-1 rounded-md border border-slate-200 bg-white px-2.5 py-1 text-xs text-slate-700">
        <Layers3 className="h-3.5 w-3.5" />
        {LEVEL_LABEL[tool.level]}
      </span>

      {statusLabel ? (
        <span className={`inline-flex items-center gap-1 rounded-md border px-2.5 py-1 text-xs ${statusLabel.className}`}>
