...
import { LogIn, LogOut, PenSquare, Shield, Wrench, X } from "lucide-react";
...
export default function HeaderClient({ navLinks }: HeaderClientProps) {
  ...
  const [session, setSession] = useState<HeaderSession | null>(() => readCachedHeaderSession() ?? null);
  const [isLoading, setIsLoading] = useState(() => readCachedHeaderSession() === undefined);
  ...
  return (
    <>
      ...
      {mobileMenuOpen ? (
        <div
          className="absolute left-0 right-0 top-[calc(100%+0.5rem)] rounded-2xl border border-slate-200 bg-white/98 p-3 md:hidden"
          style={{ boxShadow: "0 12px 24px rgba(15,23,42,0.10)" }}
        >
