...
export default function StudentToolCard({ tool }: { tool: Tool }) {
  return (
    <article
      className="relative overflow-hidden rounded-3xl border border-white/15 p-5 shadow-[0_6px_20px_rgba(0,0,0,0.22)]"
      style={{
        background:
          "linear-gradient(180deg, rgba(255,255,255,0.085) 0%, rgba(255,255,255,0.055) 100%)",
    ...
    >
      ...
      <div className="flex min-h-[230px] flex-col">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {tool.edu_verified ? (
            <span className="inline-flex items-center rounded-full border border-emerald-300/30 bg-emerald-400/10 px-2.5 py-1 text-xs font-medium text-emerald-100">
              Pack estudiante
            </span>
          ) : null}
          {tool.ia_type ? (
            <span className="inline-flex items-center rounded-full border border-white/20 bg-white/10 px-2.5 py-1 text-xs text-white/70">
              {tool.ia_type}
            </span>
          ) : null}
        </div>

        <h2 className="mt-4 text-xl font-semibold leading-snug text-white">{tool.name}</h2>
        ...
        <div className="mt-auto flex flex-wrap items-center gap-3 pt-6">
          <Link
            ...
            className="inline-flex items-center rounded-full border border-cyan-300/40 bg-cyan-400/15 px-4 py-2 text-sm font-medium text-cyan-100 transition-colors duration-150 hover:bg-cyan-400/25"
          >
            Ver detalle
          </Link>
          {tool.url ? (
            <a
              ...
              className="inline-flex items-center rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-medium text-white/80 transition-colors duration-150 hover:bg-white/15"
            >
              Ir a herramienta
            </a>
          ) : null}
